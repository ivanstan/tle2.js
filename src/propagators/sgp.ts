 ...
 * Based on SPACETRACK REPORT NO. 3, Section 5
 * 
 * This is the simplest model, used for near-Earth satellites.
 * Uses Kozai's gravitational model with linear drag effect on mean motion.
 */

import { 
  CK2, CK4, XKE, XKMPER, TWOPI, TOTHRD, AE, E6A
} from './constants';
import { OrbitalElements, StateVector, PropagationResult } from './types';
 ...
 */
export function sgp(elements: OrbitalElements, tsince: number): PropagationResult {
  const { 
    ndot, nddot, bstar,
  ...
  const a3ovk2 = -1.0 * (1.0e-6 * -0.253881) / CK2;
  ...
  const xmcof = -TOTHRD * coef * bstar * AE / eeta;
  ...
  // For SGP, we use ndot and nddot directly from TLE
  const d2 = 0.0;
  const d3 = 0.0;
  const d4 = 0.0;
  const t3cof = 0.0;
  ...
  // Use ndot/2 and nddot/6 for drag effects in SGP
  const xn = xnodp + ndot * t + nddot * tsq;
  const em = ecco + tempe;
  xmp = xmdf + xnodp * templ;
  ...
  // Clamp eccentricity
  const e = Math.max(em, 1e-6);
  
  // Long period periodics
  const axn = e * Math.cos(omega);
  const temp = 1.0 / (aodp * (1.0 - e * e));
  const xlp = temp * xlcof * axn;
  const aynl = temp * aycof;
  const xl = xmp + omega + xnode + xlp;
  ...
  const r = aodp * (1.0 - ecose);
  const temp6 = 1.0 / r;
  const rdot = XKE * Math.sqrt(aodp) * esine * temp6;
  const rfdot = XKE * Math.sqrt(pl) * temp6;
  const temp7 = 1.0 / pl;
  const betal = Math.sqrt(1.0 - elsq);
  const cosu = temp6 * (cosepw - axn + ayn * esine * temp7 / (1.0 + betal));
  const sinu = temp6 * (sinepw - ayn - axn * esine * temp7 / (1.0 + betal));
  const u = Math.atan2(sinu, cosu);
  const sin2u = 2.0 * sinu * cosu;
  const cos2u = 2.0 * cosu * cosu - 1.0;
  const temp8 = 1.0 / pl;
  const temp9 = CK2 * temp8;
  const temp10 = temp9 * temp8;
  
  // Short period periodics
  const rk = r * (1.0 - 1.5 * temp10 * betal * x3thm1) + 0.5 * temp9 * x1mth2 * cos2u;
  const uk = u - 0.25 * temp10 * x7thm1 * sin2u;
  const xnodek = xnode + 1.5 * temp10 * cosio * sin2u;
  const xinck = inclo + 1.5 * temp10 * cosio * sinio * cos2u;
  const rdotk = rdot - xn * temp9 * x1mth2 * sin2u;
  const rfdotk = rfdot + xn * temp9 * (x1mth2 * cos2u + 1.5 * x3thm1);
